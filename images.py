...
from image.image_frenchfootballweekly import (
    get_french_football_weekly_image,
    is_french_football_weekly,
)
...
def get_best_image(
    ...
    1. برای French Football Weekly از روش اختصاصی استفاده می‌شود.
    2. در French Football Weekly تصویر RSS استفاده نمی‌شود.
    3. برای سایر سایت‌ها صفحهٔ خبر طبق منطق قبلی بررسی می‌شود.
    4. سپس RSS برای سایر سایت‌ها fallback است.
    5. تصاویر لوگو و asset حذف می‌شوند.
    6. لوگوی مشخص French Football Weekly فیلتر می‌شود.
    7. حداکثر دو تصویر بدون ابعاد واقعاً بررسی می‌شوند.
    """
    ...
    #
    # این سایت روش اختصاصی خودش را دارد.
    # --------------------------------------------------------

    if (
        article_url
        and is_french_football_weekly(
            article_url
        )
    ):
        ...
        print(
            "⚠️ روش اختصاصی "
            "French Football Weekly "
            "نتوانست تصویر را پیدا کند."
        )

        print(
            "⚠️ برای این سایت تصویر RSS استفاده نمی‌شود."
        )

        return ""

    # --------------------------------------------------------
    # اول صفحهٔ خبر برای سایر سایت‌ها
    # --------------------------------------------------------
    ...
    #
    # این بخش برای اطمینان نگه داشته شده است.
    # چون حالت اصلی FFW بالاتر return می‌کند.
    # --------------------------------------------------------
